...
import { getAllFunctions, createFunction, updateFunction, deleteFunction } from "@/lib/operational-functions"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function FunctionsPage() {
  ...
  const [editingFunction, setEditingFunction] = useState<OperationalFunction | null>(null)
  ...
  const loadFunctions = async () => {
    try {
      ...
      logAccess("LOAD_FUNCTIONS", "/funcoes", false, "Failed to load functions")
      toast({
        title: "Erro",
  ...
  const handleSubmitFunction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingFunction) {
        if (!canEdit) {
          logAccess("UPDATE_FUNCTION", `/funcoes/${editingFunction.id}`, false, "No edit permission")
          return
        }
        ...
        if (!canCreate) {
          logAccess("CREATE_FUNCTION", "/funcoes", false, "No create permission")
          return
        }
      ...
      const resource = editingFunction ? `/funcoes/${editingFunction.id}` : "/funcoes"
  ...
  const handleDeleteFunction = async (functionId: string) => {
    if (!canDelete) {
      logAccess("DELETE_FUNCTION", `/funcoes/${functionId}`, false, "No delete permission")
      return
    }
    ...
    try {
      ...
      logAccess("DELETE_FUNCTION", `/funcoes/${functionId}`, false, "Failed to delete function")
      toast({
        title: "Erro",
      ...
      <div className="min-h-screen bg-muted/30 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold text-primary">Funções Operacionais</h1>
              <p className="text-muted-foreground">Gerencie as funções disponíveis para operações</p>
            </div>
          </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Lista de todas as funções operacionais disponíveis para atribuição em operações.
                  </CardDescription>
                </div>
            ...
            <CardContent>
              {isLoading ? (
                <div className="flex items-center justify-center py-8">
                  <div className="text-muted-foreground">Carregando funções...</div>
                </div>
              ) : (
                    ...
                    <TableBody>
                      {functions.map((func) => (
                        <TableRow key={func.id}>
                          ...
                          <TableCell>
                            <Badge variant={getCategoryVariant(func.category)}>{getCategoryLabel(func.category)}</Badge>
                          </TableCell>
                          <TableCell>
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Tem certeza que deseja excluir a função "{func.name}"? Esta ação não pode ser
                                        desfeita.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                      <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                      <AlertDialogAction onClick={() => handleDeleteFunction(func.id)}>
                                        Excluir
                                      </AlertDialogAction>
                                    </AlertDialogFooter>
                                  </AlertDialogContent>
          ...
          <Dialog open={isFormModalOpen} onOpenChange={setIsFormModalOpen}>
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                <DialogTitle>{editingFunction ? "Editar Função" : "Nova Função"}</DialogTitle>
                <DialogDescription>
                  {editingFunction
              ...
              <form onSubmit={handleSubmitFunction}>
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value: "entrada" | "apoio" | "comando" | "especializada") =>
                ...
                <DialogFooter>
                  ...
                  <Button type="submit" disabled={isSubmitting}>
                    {isSubmitting ? "Salvando..." : editingFunction ? "Salvar" : "Criar"}
                  </Button>
                </DialogFooter>
              </form>
            </DialogContent>
