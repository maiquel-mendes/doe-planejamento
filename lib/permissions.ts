...
import type { Permission, RolePermissions, AccessLog } from "@/types/permissions"
...
export const logAccess = (user: User, action: string, resource: string, success: boolean, details?: string): void => {
  const log: AccessLog = {
  ...
  accessLogs.push(log)
  console.log(`[ACCESS LOG] ${user.name} (${user.role}) ${action} ${resource} - ${success ? "SUCCESS" : "DENIED"}`)
}
